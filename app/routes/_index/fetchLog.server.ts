...
export default function fetchLogFromLoggerator() {
  ...
  client.on("data", async function (data: ReadableStream) {
    ...
    const logObjects: Array<Log | null> = logLines
      .map(parseLog)
      .filter((l) => l);
    ...
    if (now - tickTime > 1000) {
      ...
      console.log(
        `Processed ${count} logs. ${logsSec} logs/sec. ${avgLogsSec} avg logs/sec`
      );
      tickCount = count;
      tickTime = now;
    }
